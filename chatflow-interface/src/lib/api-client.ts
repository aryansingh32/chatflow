...
export const api = {
  ...
  resumeJob: (jobId: string, input: string) => post<{ jobId: string; resumed: boolean }>(`/jobs/${jobId}/resume`, { input }),
  cancelJob: (jobId: string) => post<{ jobId: string; cancelled: boolean }>(`/jobs/${jobId}/cancel`, {}),
  ...
  getProfile: (profileName: string, userId?: string) =>
    get<{ profile: UserProfile }>(`/memory/profiles/${encodeURIComponent(profileName)}?userId=${userId || config.userId}`),
  ...
  updateProfile: (profileName: string, data: Record<string, string>, newName?: string, userId?: string) =>
  ...
  deleteProfile: (profileName: string, userId?: string) =>
    del<{ deleted: boolean }>(`/memory/profiles/${encodeURIComponent(profileName)}?userId=${userId || config.userId}`),
  ...
  listFiles: (category?: string, userId?: string) =>
    get<{ files: UserFile[] }>(`/files?userId=${userId || config.userId}${category ? `&category=${category}` : ''}`),
  ...
  getWorkflow: (workflowId: string) =>
    get<{ workflow: SiteWorkflow }>(`/workflows/${workflowId}`),
