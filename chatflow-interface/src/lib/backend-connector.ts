...
import { socketService, type PauseEvent, type FileEvent, type JobStartedEvent } from './socket-service';
      ...
      if (cardKind) {
        ...
        emitter.pushMessage({
          ...
          data: event.type === 'captcha'
            ...
            ? { inputType: event.type === 'password' ? 'password' : event.type === 'email' ? 'email' : 'text' }
            : undefined,
        });
      } else {
...
export async function resolveCard(id: string, value: string, jobId?: string): Promise<void> {
  ...
  if (typeof window !== 'undefined') {
    window.dispatchEvent(
      new CustomEvent('agent-card-resolve', { detail: { id, value } })
    );
  }
