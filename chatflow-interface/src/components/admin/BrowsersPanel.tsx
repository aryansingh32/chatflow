...
export function BrowsersPanel() {
  ...
  const load = useCallback(async () => {
    try { const r = await adminApi.browsers(); setStats(r.browsers); } catch {} finally { setLoading(false); }
  }, []);
  ...
  const handleRecycle = async () => {
    ...
    setTimeout(() => { setRecycling(false); load(); }, 2000);
  };
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <StatCard title="Total Browsers" value={total} icon={<Server className="h-5 w-5" />} color="violet" />
        <StatCard title="Active Leases" value={active} icon={<Monitor className="h-5 w-5" />} color="blue" pulse={active > 0} />
        <StatCard title="Contexts" value={contexts} icon={<Server className="h-5 w-5" />} color="cyan" />
        <StatCard title="Idle" value={Math.max(0, total - active)} icon={<Server className="h-5 w-5" />} color="green" />
      </div>

      {/* Pool config */}
      <div className="rounded-2xl border border-border/40 bg-card/40 p-5">
        <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-3">Pool Configuration</p>
        <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
          {Object.entries(stats).filter(([k]) => !["totalBrowsers", "activeLeasesCount", "totalContexts"].includes(k)).map(([key, val]) => (
