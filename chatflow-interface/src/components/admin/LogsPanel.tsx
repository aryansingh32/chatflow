  ...
  return (
    <div className="flex flex-col gap-4" style={{ height: "calc(100vh - 160px)" }}>
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          <div
            className={`relative h-5 w-9 rounded-full transition-colors ${autoRefresh ? "bg-violet-500" : "bg-zinc-700"}`}
          >
            <div
...
export function ErrorsPanel() {
  ...
  const load = useCallback(async () => {
    try {
      const r = await adminApi.getErrors(200);
      setErrors(r.errors);
    } catch {
    } finally {
      setLoading(false);
  ...
  return (
    <div className="space-y-4">
      ...
      {loading && !errors.length ? (
        ...
        <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 py-10 text-center text-sm text-emerald-400">
          ✓ No recent errors
        </div>
      ) : (
        errors.map((e, i) => (
          <div key={i} className="rounded-xl border border-red-500/20 bg-red-500/5 px-4 py-3">
            <div className="flex items-start justify-between gap-2">
              <p className="text-sm text-red-300">
                {String(e.message ?? e.msg ?? "").slice(0, 200)}
              </p>
