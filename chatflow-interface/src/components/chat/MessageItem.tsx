...
import { ChevronDown, Check, Loader2, Circle, Download, FileText, Paperclip, Info, AlertTriangle, CheckCircle2, XCircle } from "lucide-react";
import type { ChatMessage, TimelineMessage, FileMessage, DownloadMessage, TextMessage, InputCardMessage, StatusMessage } from "@/lib/chat-types";
        ...
        <ol className="space-y-2 border-t border-border px-4 py-3 text-sm">
          {msg.steps.map((s) => (
            <li key={s.id} className="flex items-center gap-2.5">
              ...
              <span className={s.status === "done" ? "text-muted-foreground line-through" : s.status === "active" ? "text-foreground" : s.status === "error" ? "text-destructive" : "text-muted-foreground"}>
                {s.label}
              </span>
            </li>
          ))}
...
export function TypingIndicator() {
  return (
    <div className="px-4 py-3">
      <div className="mx-auto flex max-w-3xl gap-3">
        <div className="mt-0.5 grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-primary/15 text-primary text-[11px] font-bold animate-pulse">A</div>
