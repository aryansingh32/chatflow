...
export class ProxyManager {
  ...
  async getProxyByTag(tag: string): Promise<ProxyConfig | null> {
    const proxies = await this.getActiveProxies();
  ...
  async healthCheckAll(): Promise<void> {
    const pool = getPgPool();
    const { rows } = await pool.query(
      `SELECT id, host, port, protocol, username, password FROM proxies WHERE is_active = true`
    );
  ...
  async addProxy(proxy: Omit<ProxyConfig, 'id' | 'healthScore' | 'latencyMs' | 'failureRate' | 'lastChecked'>): Promise<string> {
    const pool = getPgPool();
    const { rows } = await pool.query(`
  ...
  async importProxies(
    list: Array<{ host: string; port: number; username?: string; password?: string; protocol?: string; tags?: string[] }>
  ): Promise<number> {
    let count = 0;
    for (const p of list) {
      await this.addProxy({
        ...
        username: p.username,
        password: p.password,
        protocol: (p.protocol ?? 'http') as ProxyConfig['protocol'],
        tags: p.tags ?? [],
      });
      count++;
    }
  ...
  private async getActiveProxies(): Promise<ProxyConfig[]> {
    ...
    if (cached) return JSON.parse(cached) as ProxyConfig[];
    ...
    const proxies: ProxyConfig[] = rows.map((r) => ({
      id:          r.id,
      host:        r.host,
      port:        r.port,
      username:    r.username,
  ...
  private async pingProxy(proxy: { host: string; port: number; protocol: string }): Promise<{ alive: boolean; latencyMs: number }> {
