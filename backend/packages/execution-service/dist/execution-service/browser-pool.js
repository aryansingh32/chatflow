...
export class BrowserPool extends EventEmitter {
    ...
    async init() {
        ...
        await Promise.all(Array.from({ length: this.config.minBrowsers }, () => this.spawnBrowser()));
    ...
    // ─── Acquire / Release ──────────────────────────────────────
    async acquireContext(sessionId, userId, session, proxy, lightweight) {
        ...
        context._isLightweight = lightweight;
        const lease = {
            contextId: randomId(),
    ...
    async releaseContext(contextId, saveSession = true) {
    ...
    async releaseContextBySessionId(sessionId, saveSession = true) {
        const existing = [...this.contexts.values()].find((c) => c.sessionId === sessionId);
        if (existing) {
            await this.releaseContext(existing.contextId, saveSession);
        }
    }
    async getOrCreatePage(contextId) {
        const lease = this.contexts.get(contextId);
        if (!lease)
        ...
        if (!lease.context._isLightweight) {
    ...
    async findOrSpawnBrowser() {
        // Find healthy browser with capacity
        const available = [...this.browsers.values()].find((b) => b.isHealthy && b.contextCount < this.config.maxContextsPerBrowser);
        if (available)
            return available;
        ...
        for (let i = 0; i < 100; i++) {
            await sleep(100);
            const retry = [...this.browsers.values()].find((b) => b.isHealthy && b.contextCount < this.config.maxContextsPerBrowser);
            if (retry)
                return retry;
        }
        throw new Error('[BrowserPool] No browser capacity available after 10s');
    }
    async createContext(browser, session, proxy) {
    ...
    async applyStealthSettings(page) {
        ...
        await page.addInitScript((profile) => {
            ...
            AnalyserNode.prototype.getFloatFrequencyData = function (array) {
                originalAnalyser.call(this, array);
                if (array.length > 0)
                    array[0] = array[0] + profile.audioNoise;
            };
    ...
    startHealthCheck() {
        this.healthCheckInterval = setInterval(async () => {
            ...
            for (const [id, browser] of this.browsers) {
                ...
                // Memory check > 3GB
                const pid = browser.browser.process?.()?.pid;
                if (pid) {
                    try {
            ...
            for (const [contextId, lastUsed] of this.contextLastUsed) {
                if (now - lastUsed > this.config.contextIdleTimeoutMs) {
                    ...
                    if (lease) {
                        ...
                        if (browser)
                            browser.contextCount = Math.max(0, browser.contextCount - 1);
                        this.contexts.delete(contextId);
                        this.contextLastUsed.delete(contextId);
                    }
                }
...
function createFingerprintProfile() {
    ...
    return {
        ...
        colorScheme: pickOne(['light', 'dark']),
        webgl: {
