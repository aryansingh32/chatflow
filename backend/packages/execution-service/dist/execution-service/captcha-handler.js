...
export async function detectCaptcha(page) {
    return page.evaluate(() => {
        ...
        if (document.querySelector('script[src*="recaptcha/api.js"]')) {
            return { detected: true, type: 'recaptcha-v3' };
        }
        // hCaptcha
...
export async function simulateHumanPresence(page) {
...
export class OpenSourceSolver {
    ...
    async solveRecaptchaV2(page, sitekey) {
        try {
            ...
            return { solved: false, method: 'open-source', error: err.message };
        }
    }
    ...
    async solveSlider(page, sliderSelector = '[class*="slider"]') {
        try {
            const slider = page.locator(sliderSelector).first();
            const box = await slider.boundingBox();
            if (!box)
                return { solved: false, method: 'open-source', error: 'Slider not found' };
...
export async function requestManualIntervention(jobId, pageUrl, captchaType) {
...
export class CaptchaHandler {
    solver = new OpenSourceSolver();
    async handle(page, jobId) {
