...
export class UserFileStore {
    ...
    async resolveInputReference(userId, rawValue) {
        ...
        if (normalized.startsWith('file:')) {
            ...
            if (!file)
                throw new Error(`Referenced file not found: ${fileId}`);
            return file.storagePath;
        }
        if (normalized.startsWith('file-category:')) {
            const category = normalized.slice('file-category:'.length);
            const file = await this.getLatestByCategory(userId, category);
            if (!file)
                throw new Error(`No uploaded file found for category: ${category}`);
            return file.storagePath;
        }
        return normalized;
    }
        ...
        try {
         ...
         RETURNING
           ...
           updated_at as "updatedAt"`, [
            ...
            throw new Error(`File persistence failed: ${err.message}`);
        }
    }
    async getFileById(fileId, userId) {
       ...
       LIMIT 1`, [fileId, userId]);
        return rows[0] ? mapRow(rows[0]) : null;
    }
    async getLatestByCategory(userId, category) {
        const pool = getPgPool();
        const { rows } = await pool.query(`SELECT
         id, user_id as "userId", profile_name as "profileName", category,
         original_name as "originalName", stored_name as "storedName",
     ...
     */
    async cleanupTempFiles(maxAgeMs = 3600 * 1000) {
        const now = Date.now();
        const tempDirs = [
