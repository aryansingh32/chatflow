...
//   1. Stored selector (by confidence rank)
//   2. Text-based matching
//   3. DOM similarity heuristic
//   4. AI semantic re-identification
// Tracks failures and auto-demotes broken selectors.
// ============================================================

export type SelectorResolution =
  | { found: true;  selector: string; method: SelectorCandidate['type']; confidence: number }
  | { found: false; method: 'exhausted'; error: string };

// ─── Resolution Context ───────────────────────────────────────

export interface ResolutionContext {
  page: Page;
  elementId: string;           // DB element id
  label: string;               // human-readable label
  elementType: ExtractedElement['type'];
  aiResolver?: AIResolver;     // injected AI fallback
}

export type AIResolver = (
  page: Page,
  label: string,
  elementType: string,
  context?: string
...
export class SelectorEngine {
  ...
  async resolve(ctx: ResolutionContext): Promise<SelectorResolution> {
  ...
  private async tryTextMatch(
    ...
    // ARIA label match
    try {
      const locator = page.getByRole(elementType as any, { name: label, exact: false });
      const count = await locator.count();
      if (count > 0) {
  ...
  private async tryHeuristicMatch(
    page: Page,
    label: string,
    elementType: string
  ): Promise<string | null> {
      ...
      ({ label, elementType }) => {
        ...
        for (const el of elements) {
          ...
          if (
            ...
            if (elName) return `${el.tagName.toLowerCase()}[name="${elName}"]`;
  ...
  private async tryAIResolver(ctx: ResolutionContext): Promise<string | null> {
    if (!this.aiResolver) return null;
  ...
  private async loadCandidates(elementId: string): Promise<SelectorCandidate[]> {
    ...
    return rows.map((r) => ({
      value: r.value,
      type: r.type as SelectorCandidate['type'],
  ...
  private async recordSuccess(elementId: string, selectorValue: string): Promise<void> {
  ...
  private async recordFailure(elementId: string, selectorValue: string): Promise<void> {
    await getPgPool().query(`
      UPDATE selectors
  ...
  private async storeNewSelector(
    elementId: string,
    value: string,
    type: SelectorCandidate['type'],
    confidence: number
...
export async function getSelectorHealthReport(siteId: string) {
  const pool = getPgPool();
  const { rows } = await pool.query(`
    SELECT
      e.type,
